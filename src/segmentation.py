...
def extract_words(img, visual=0, cut=3, save_imgs=0):
    lines = line_horizontal_projection(img)
    words = []

    for idx, line in enumerate(lines):
        if visual and save_imgs == 1:
            save_image(line, "lines", f"line{idx}")
        # * Updated by : NTI Post Graduate Diploma 2023/2024  (Arabic Recognition Project) cut
        line_words = word_vertical_projection(line, cut)
        for w in line_words:
            # if len(words) == 585:
            #     print(idx)
            words.append((w, line))
